<?php 
    if (isset($_GET['tipo_id'])) {
        ...
        $produto = new Produto();
        $produtos = $produto->buscarPorTipo($tipo_id);
...
<body class="fundo-fixo">
    ...
    <main class="container shadow bg-light px-5">
        ...
        <section>
        <?php if ($linhas == 0) { ?>
            <h2 class="alert bg-dark text-white text-center">Não há produtos cadastrados.</h2>
        <?php }?>
        <?php if($linhas > 0) {?>
            <h2 class="alert bg-dark text-white text-center">Produtos</h2>
            <div class="row">
                <!-- Card -->
